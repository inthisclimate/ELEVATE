...
getwd()
df <-read.csv("Efflux/Jan10-S4.CO2/TG10-01977-2025-01-10T083000.data.csv")
...
#CO2 is in ppm
df$RATE_CO2 <- c(NA, diff(df$CO2) / diff(df$SECONDS)) #Estimate the rate of change with the difference between CO2 & time passing; add NA to fix mismatch
summary(df$RATE_CO2, na.rm=TRUE) #avg 0.1297ppm/sec
df$RATE_CO2_molmol <- df$RATE_CO2 * 1e-6
summary(df$RATE_CO2_molmol, na.rm=TRUE) #avg 1.00e-07 mol/mol/sec

#CH4 is in ppb
df$RATE_CH4 <- c(NA,diff(df$CH4) / diff(df$SECONDS)) #Estimate the rate of change with the difference between CO2 & time passing; add NA to fix mismatch
summary(df$RATE_CH4, na.rm=TRUE) #avg  0.0036ppb/sec
...
climate <- aranet.jan%>%filter(site=="S4"& treatment=="Elevated"& time_CST >"2025-01-10 11:34:01 CST" & time_CST < "2025-01-10 14:50:00 CST") 
summary(climate)
...
write.csv(df.fluxes, "Efflux/efflux_cleaned/S4.elevated.fluxes.csv", row.names = FALSE)
...
ggplot(df.fluxes, aes(x = CO2_flux.umol.m2.s)) +
  ...
  theme_minimal()
ggsave("Efflux/efflux_cleaned/S4.elevated.co2hist.jpg")
...
ggplot(df.fluxes, aes(x = CH4_flux.umol.m2.s)) +
  ...
  theme_minimal()
ggsave("Efflux/efflux_cleaned/S4.elevated.ch4hist.jpg")
...
ggplot(df.fluxes, aes(x = time_elapsed, y = CO2_flux.umol.m2.s)) +
  ...
  theme_grey()
ggsave("Efflux/efflux_cleaned/S4.elevated.co2.jpg")
...
ggplot(df.fluxes, aes(x = time_elapsed, y = CH4_flux.umol.m2.s)) +
  ...
  theme_grey()
ggsave("Efflux/efflux_cleaned/S4.elevated.ch4.jpg")
